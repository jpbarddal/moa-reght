...
public class FIMTDDReg extends AbstractClassifier implements Regressor {
	...
	public abstract static class Node extends AbstractMOAObject {
		...
		public Node(FIMTDDReg tree, double lambda) {
			this.tree = tree;
			ID = tree.maxID;
			this.lambda = lambda;
		}
	...
	public static class LeafNode extends Node {
		 ...
		 */
		public LeafNode(FIMTDDReg tree, double lambda) {
			super(tree, lambda);
		...
		public void checkForSplit(FIMTDDReg tree) {
	...
	public abstract static class InnerNode extends Node {
		...
		public InnerNode(FIMTDDReg tree, double lambda) {
			super(tree, lambda);
		}
	...
	public static class SplitNode extends InnerNode {
		...
		// the merits of the features selected
		 ...
		 * @param tree 
		 */
		public SplitNode(InstanceConditionalTest splitTest, FIMTDDReg tree, double lambda, int indicesFeaturesSelected[],
	...
	public class FIMTDDPerceptron implements Serializable {
		...
		public FIMTDDPerceptron(FIMTDDReg tree) {
			this.tree = tree;
			reset = true;
		}
		...
		private double denormalizePrediction(double normalizedPrediction, FIMTDDReg tree) {
	...
	protected SplitNode newSplitNode(InstanceConditionalTest splitTest, double lambda,
									 int indicesFeaturesSelected[], double meritsFeaturesSelected[]) {
		maxID++;
		return new SplitNode(splitTest, this, lambda, indicesFeaturesSelected, meritsFeaturesSelected);
	}

	protected LeafNode newLeafNode() {
		maxID++;
		return new LeafNode(this, lambdaOption.getValue());
	}
	...
	protected void attemptToSplit(LeafNode node, Node parent, int parentIndex) {
		...
		if(parent != null) {
			indicesSelected = new int[((SplitNode)parent).indicesFeaturesSelected.length + 1];
			System.arraycopy(((SplitNode)parent).indicesFeaturesSelected, 0, indicesSelected, 0, ((SplitNode)parent).indicesFeaturesSelected.length);
			meritsSelected = new double[((SplitNode)parent).meritsFeaturesSelected.length + 1];
			System.arraycopy(((SplitNode)parent).meritsFeaturesSelected, 0, meritsSelected, 0, ((SplitNode)parent).meritsFeaturesSelected.length);
		}else{
		...
		if (bestSplitSuggestions.length < 2) {
			...
			if (useMinMeritForSplitOption.isSet() &&
					bestSuggestion.merit > minValueForSplit &&
		...
		if (shouldSplit) {
			...
			SplitNode newSplit = newSplitNode(splitDecision.splitTest, lambdaOption.getValue(), indicesSelected, meritsSelected);
